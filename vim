...
---|--------|------------
   |:1234:  h	| 	left (also: CTRL-H, <BS>, or <Left> key)
   |:1234:  l	| 	right (also: <Space> or <Right> key)
...
Status | Command | Description
---|--------|------------
   | :1234:  k		| up N lines (also: CTRL-P and <Up>)
   | :1234:  j		| up N lidown N lines (also: CTRL-J, CTRL-N, <NL>, and <Down>)
   ...
   | :1234:  %		| up N ligoto line N percentage down in the file; N must be given, otherwise it is the |%| command
   | :1234:  gk		| up N liup N screen lines (differs from "k" when line wraps)
   | :1234:  gj		| up N lidown N screen lines (differs from "j" when line wraps)
...
---|--------|------------
   ...
   | :1234:  W		| N blank-separated |WORD|s forward
   ...
   | :1234:  B		| N blank-separated |WORD|s backward
...
---|--------|------------------------------
    ...
    | :1234:  A	| append text at the end of the line (N times)
    | :1234:  i	| insert text before the cursor (N times) (also: <Insert>)
    | :1234:  I	| insert text before the first non-blank in the line (N times)
    | :1234:  gI	| insert text in column 1 (N times)
    | :1234:  o	| open a new line below the current line, append text (N times)
    | :1234:  O	| open a new line above the current line, append text (N times)
...
---|--------|------------------------------
    ...
    | :1234:  J		| join N-1 lines (delete <EOL>s)
...
---|--------|------------------------------
   ...
   | :1234:  P		| put a register before the cursor position (N times)
...
Status | Command | Description
---|--------|------------------------------
    | :1234:  r{char}	| replace N characters with {char}
...
---|--------|------------------------------
    ...
    | :1234:  ~		| switch case for N characters and advance cursor
...
Status | Command | Description
---|--------|------------------------------
   | :1234:  .		 | repeat last change (with count replaced with N)
