...
function isTruthy(value: Value): boolean {
  if (value.type === 'number' || value.type === 'float') {
...
class IfElseBlock {
  ...
  public async execute(vimState: VimState): Promise<CommandResult> {
    ...
    for (const branch of this.branches) {
      if (isTruthy(ctx.evaluate(branch.condition))) {
...
class WhileBlock {
  ...
  public async execute(vimState: VimState): Promise<CommandResult> {
    const ctx = this.script.evalContext;
    while (isTruthy(ctx.evaluate(this.branch.condition))) {
...
export class Script {
  ...
  private _evalContext!: EvaluationContext;
