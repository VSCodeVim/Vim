...
class ForBlock {
  ...
  public async execute(vimState: VimState): Promise<CommandResult> {
    ...
    if (
      ...
      throw VimError.fromCode(ErrorCode.StringListOrBlobRequired);
    }
...
export class Script {
  ...
  private _evalContext = new EvaluationContext(this);
  ...
  private parse(): Array<Command | Block> {
    ...
    for (const line of this.lines) {
      ...
      if (command instanceof NoOpCommand) {
        ...
        if (!(block instanceof IfElseBlock)) {
          throw VimError.fromCode(ErrorCode.ElseIfWithoutIf);
        }
        block.addBranch(command.condition);
      } else if (command instanceof ElseCommand) {
        if (!(block instanceof IfElseBlock)) {
          throw VimError.fromCode(ErrorCode.ElseWithoutIf);
        }
        block.addBranch(bool(true));
      } else if (command instanceof EndIfCommand) {
        if (!(block instanceof IfElseBlock)) {
          throw VimError.fromCode(ErrorCode.EndIfWithoutIf);
        }
        finishBlock();
        ...
        if (!(block instanceof FunctionBlock)) {
          throw VimError.fromCode(ErrorCode.EndFunctionNotInsideFunction);
        }
        finishBlock();
      } else {
    ...
    if (notClosed instanceof IfElseBlock) {
      throw VimError.fromCode(ErrorCode.MissingEndIf);
    } else if (notClosed instanceof WhileBlock) {
      throw VimError.fromCode(ErrorCode.MissingEndWhile);
    } else if (notClosed instanceof ForBlock) {
      throw VimError.fromCode(ErrorCode.MissingEndWhile); // TODO: MissingEndFor
    } else if (notClosed instanceof FunctionBlock) {
      throw VimError.fromCode(ErrorCode.MissingEndFunction);
    }
  ...
  async execute(vimState: VimState): Promise<void> {
    this._evalContext = new EvaluationContext(this);
