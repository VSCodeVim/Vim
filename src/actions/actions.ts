...
import {
  waitForCursorUpdatesToHappen,
  getAllCursors } from '../util';
...
export interface IMovement {
  /**
   * The location the cursor will end up.
   */
  cursorPosition : Position;

  /**
   * The range we will supply to an operator in the future, if one is run.
   */
...
export function isIMovement(o: IMovement | Position): o is IMovement {
    return (o as IMovement).cursorPosition !== undefined &&
           (o as IMovement).operatingRange !== undefined;
}
      ...
      for (let i = 0; i < count; i++) {
          ...
          if (temporaryResult instanceof Position) {
            ...
            if (result instanceof Position) {
              ...
              // TODO(johnfn): Study - does this code even ever get run..?
              result = {
                cursorPosition : new Position(0, 0),
                operatingRange : new Range(new Position(0, 0), new Position(0, 0)),
                failed         : false
              };
            ...
            if (firstIteration) {
              result.operatingRange = new Range(result.cursorPosition, result.cursorPosition);
            }

            if (lastIteration) {
              result.operatingRange = result.operatingRange.withNewStop(temporaryResult.cursorPosition);
            }

            if (!lastIteration) {
              position = temporaryResult.cursorPosition.getRightThroughLineBreaks();
            }
          }
      }
...
export abstract class BaseCommand extends BaseAction {
  ...
  public async execCount(position: Position, vimState: VimState): Promise<VimState> {
    ...
    vimState.allCursors = await vimState.allCursors.asyncMap(async (cursor, i) => {
      this.multicursorIndex = i;

      vimState.allCursors = new EqualitySet([ cursor ]);

      for (let j = 0; j < timesToRepeat; j++) {
        vimState = await this.exec(cursor.getStop(), vimState);
      }

      return new Range(vimState.cursorStartPosition, vimState.cursorPosition);
    });
...
class CommandEsc extends BaseCommand {
  ...
  public async exec(position: Position, vimState: VimState): Promise<VimState> {
    if (vimState.currentMode !== ModeName.Visual &&
      ...
      vimState.allCursors = vimState.allCursors.map<Range>(x => x.getLeft());
    }
    ...
    if (vimState.currentMode === ModeName.Normal && vimState.isMultiCursor) {
      ...
      vimState.allCursors = new EqualitySet([ vimState.allCursors.first() ]);
    }
...
export class DeleteOperator extends BaseOperator {
    ...
    public async delete(start: Position, end: Position, currentMode: ModeName,
                        registerMode: RegisterMode, vimState: VimState, yank = true): Promise<void> {
      ...
      let diff = PositionDiff.NewDiff(0, 0);
...
export class YankOperator extends BaseOperator {
    ...
    public async run(vimState: VimState, start: Position, end: Position): Promise<VimState> {
      ...
      if (originalMode === ModeName.Normal) {
        vimState.allCursors = vimState.cursorPositionsJustBeforeAnythingHappened.map<Range>(x => new Range(x, x));
      } else {
        vimState.cursorPosition = start;
      }
...
class CommandUndo extends BaseCommand {
  ...
  public async exec(position: Position, vimState: VimState): Promise<VimState> {
    ...
    if (newPositions !== undefined) {
      vimState.allCursors = newPositions.map<Range>(x => new Range(x, x));
    }
...
class CommandVisualMode extends BaseCommand {
  ...
  public async exec(position: Position, vimState: VimState): Promise<VimState> {
    ...
    console.log('go to visual mode!');
    console.log(vimState.allCursors.toString());
...
class CommandInsertNewLineAbove extends BaseCommand {
  ...
  public async exec(position: Position, vimState: VimState): Promise<VimState> {
    ...
    vimState.allCursors = getAllCursors();
...
class MoveFindForward extends BaseMovement {
  ...
  public async execActionWithCount(position: Position, vimState: VimState, count: number): Promise<Position | IMovement> {
    ...
    if (!result) {
      return { cursorPosition: position, operatingRange: new Range(position, position), failed: true };
    }
...
abstract class MoveByScreenLine extends BaseMovement {
  ...
  public async execAction(position: Position, vimState: VimState): Promise<Position | IMovement> {
    ...
    if (vimState.currentMode === ModeName.Normal) {
      ...
      return {
        cursorPosition: stop,
        operatingRange: new Range(start, stop),
      };
    }
  }
...
class MoveNonBlankLast extends BaseMovement {
  ...
  public async execActionWithCount(position: Position, vimState: VimState, count: number): Promise<Position | IMovement> {
    ...
    return {
      cursorPosition: stop,
      operatingRange: new Range(vimState.cursorStartPosition, stop),
      registerMode: RegisterMode.LineWise,
    };
  }
}
...
class ActionXVisualBlock extends BaseCommand {
  ...
  public async exec(position: Position, vimState: VimState): Promise<VimState> {
    ...
    vimState.allCursors = new EqualitySet([ new Range(position, position) ]);
...
class MoveDD extends BaseMovement {
  ...
  public async execActionWithCount(position: Position, vimState: VimState, count: number): Promise<Position | IMovement> {
    ...
    let stop  = position.getDownByCount(Math.max(0, count - 1)).getLineEnd();
...
class MoveCC extends BaseMovement {
  ...
  public async execActionWithCount(position: Position, vimState: VimState, count: number): Promise<IMovement> {
    let start = position.getLineBeginRespectingIndent();
    let stop  = position.getDownByCount(Math.max(0, count - 1)).getLineEnd();
...
abstract class TextObjectMovement extends BaseMovement {
  ...
  public async execActionForOperator(position: Position, vimState: VimState): Promise<IMovement> {
    ...
    res.operatingRange = res.operatingRange.withNewStop(
      new Position(res.operatingRange.getStop().line,
                   res.operatingRange.getStop().character + 1));
...
class MoveToMatchingBracket extends BaseMovement {
  ...
  public async execAction(position: Position, vimState: VimState): Promise<Position | IMovement> {
    ...
    const failure = { cursorPosition: position, operatingRange: new Range(position, position), failed: true };
...
abstract class MoveInsideCharacter extends BaseMovement {
  ...
  public async execAction(position: Position, vimState: VimState): Promise<Position | IMovement> {
    ...
    return {
      ...
      operatingRange: new Range(startPos, endPos),
    };
  }
}
...
abstract class MoveQuoteMatch extends BaseMovement {
  ...
  public async execAction(position: Position, vimState: VimState): Promise<IMovement> {
    ...
    if (start === -1 || end === -1 || end === start || end < position.character) {
      return {
        cursorPosition: position,
        operatingRange: new Range(position, position),
        failed: true
      };
    }
  ...
  public async execActionForOperator(position: Position, vimState: VimState): Promise<IMovement> {
    ...
    res.operatingRange.withNewStop(res.operatingRange.getStop().getRight());
...
abstract class MoveTagMatch extends BaseMovement {
  ...
  public async execActionForOperator(position: Position, vimState: VimState): Promise<IMovement> {
    ...
    res.operatingRange = res.operatingRange.withNewStop(res.operatingRange.getStop().getRight());
